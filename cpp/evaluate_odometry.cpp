...
vector<errors> calcSequenceErrors (vector<Matrix> &poses_gt,vector<Matrix> &poses_result) {
  ...
  for (int32_t first_frame=0; first_frame<poses_gt.size(); first_frame+=step_size) {
    ...
    for (int32_t i=0; i<num_lengths; i++)
    {
      ...
      //cout << "last_frame = " << last_frame << " first frame = " << first_frame << endl;
...
//      cout << "last_frame = " << last_frame << " first frame = " << first_frame << \
//              " 0.1*num_frames = " << 0.1*num_frames << " speed = " << speed << endl;
//      cout << "x = " << poses_gt[first_frame].val[0][3] << "val[3][3] = " << poses_gt[first_frame].val[3][3] << endl;
      
      // write to file
      err.push_back(errors(first_frame,r_err/len,t_err/len,len,speed));
      cout << "first_frame = " << first_frame << " last_frame = " << last_frame << " r_err/len = " << r_err/len \
           << " t_err/len = " << t_err/len << " len = " << len << " speed = " << speed << endl;
    }
  }
